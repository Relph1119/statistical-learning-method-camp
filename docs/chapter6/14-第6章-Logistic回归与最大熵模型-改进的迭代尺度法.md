# 第6章-Logistic回归与最大熵模型-改进的迭代尺度法{docsify-ignore-all}
## 改进的迭代尺度法
...
&emsp;&emsp;上述问题变为：  
已知$\displaystyle g(\delta_i) = \sum_{x,y} \tilde{P}(x) P_w(y|x)f_i(x,y) \exp (\delta_i f^\#(x,y)) - E_{\tilde{P}}(f_i)$，求解$\delta_i$，使得$g(\delta_i) = 0$。&emsp;&emsp;      
